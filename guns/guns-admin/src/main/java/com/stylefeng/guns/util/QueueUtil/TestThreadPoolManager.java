...
//        System.out.println("此订单准备添加到线程池，订单号：" + SQL);
        //验证当前进入的订单是否已经存在
//        if (cacheMap.get(SQL) == null) {
            cacheMap.put(SQL, new Object());
            BusinessThread businessThread = new BusinessThread(SQL,dao);
        ...
        public void run() {
            ...
            if(!msgQueue.isEmpty()){
                ...
                if (threadPool.getQueue().size() < WORK_QUEUE_SIZE) {
                    ...
                    BusinessThread businessThread = new BusinessThread(orderId,dao);
                    threadPool.execute(businessThread);
